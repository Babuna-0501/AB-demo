...
window.GALAXY = {
	...
	dom: {
		header: $('.gnb-header').get(0) || $('header').get(0) || $('#gnb-navbar').get(0)
	},
	...
	header: function() {
		...
		if (!$(GALAXY.isGalaxy?'.gnb-header':GALAXY.dom.header).length) {
			return;
		}

		var
			$header = $(GALAXY.isGalaxy?'.gnb-header':GALAXY.dom.header),
		...
		subNav = (function() {
			...
			function setScroll(noRefresh) {
				...
				if (iScroll) {
					...
					if(isType1){
						if ( $links[current][0].offsetLeft+$($links[current][0]).outerWidth() > GALAXY.areaWidth ){
							var _left = ($links[current][0].offsetLeft+$($links[current][0]).outerWidth()) - GALAXY.areaWidth ;
							// 7은 subnavi 좌우 그라디언트 여백 값
							iScroll.scrollTo(-_left-7, 0);
						}
					}
				} else {
					if (noRefresh!==true) {
				...
				setSizeMode: function(sizeMode) {
					for (var newSrc, i = 0, max = $figures.length; i < max; i++) {
						...
						if ($images[i][0] && $images[i][0].src && ($images[i][0].src.indexOf(newSrc.replace('./','')) == -1)) {
							$($images[i][0]).attr("data-src", GALAXY.setMediaBaseURL(newSrc)).removeClass('lazyloaded').addClass('lazyload lazypreload');
						}
						if (hasVideo) {
			...
			if (!$layer) {
				$layer = $([
					'<div id="layer-youtube" class="m_layer-youtube" tabindex="0"><div class="m_ly_inner">',
						...
						'</div>',
						'<button type="button" class="c_btn_close-type1">Youtube Close</button>',
					'</div></div>'
				].join(''));
	...
	noScroll: (function() {
		...
		return {
			...
			off: function() {
				...
				if ( $('#subnav .subnav-menus').length && GALAXY.areaWidth < 1920+scrollBarWidth) {
					$('#subnav .subnav-menus ul').css('padding-right', '' );
...
$(window).load(function() {
	...
	$(window).on('load', function () {
		$(function () {
			...
			var oneRowHeight = 20,
				$footer_left = $('.m_dotcom_footer article .footer-left'),
				$footer_disclaimer_p = $('.m_dotcom_footer article .footer-bottom.bottom-sticky-nav-disclaimer'),
				$footer_disclaimer_li = $('.m_dotcom_footer article .footer-bottom #desc-section ol');

			var $footer_left_content = $footer_left.length > 0 ? $footer_left.children() : '',
				$footer_disclaimer_p_content = $footer_disclaimer_p.length > 0 ? $footer_disclaimer_p.children() : '',
				$footer_disclaimer_li_content = $footer_disclaimer_li.length > 0 ? $footer_disclaimer_li.children() : '';

			var prepare = function() {
				var formatElement = function (root, content, li) {
					if(!content) return;
					var li = li || false;
					var len = content.length;
					// var height = oneRowHeight || 20;
					var newContent = '';
					_.forEach(content, function (item) {
						var html = $(item).html();
						if(html.indexOf('<a') > -1 && html.indexOf('<a') <= 80) {
                            if (li) {
                                html = '<li>' + html + '</li>'
                            }
                            newContent += html;
						...
						if($(item).height() > oneRowHeight) {
							html = '<div class="disclaimer__single" data-default>' + $(item).html() + '</div>';
                            if (spanish) {
                                html = '<div class="disclaimer__row">' + html + '<a class="disclaimer__hide__single">Mostrar Menos</a> <a class="disclaimer__show__single">Mostrar Más</a></div>';
                            }
                            else {
								html = '<div class="disclaimer__row">' + html + '<a class="disclaimer__hide__single">Show Less</a> <a class="disclaimer__show__single">Show more</a></div>';
						    }
                        }
						if (li) {
							html = '<li>' + html + '</li>'
			...
			var init = function() {
				...
				var $disclaimer = $('.disclaimer__row'),
					...
					// disclaimerWidth = $disclaimer[0].offsetWidth,
					rowWidth = 0;
				var formatDisclaimer = function ($item, resize, idx) {
					...
					// console.log('---limitLength---', idx, limitLength);
					if (strings.length > 0) {
						var temp = $item.html();
						$item.text('Abcd efgh ');
                        ...
                        // if(stringLength >= htmlStrings.length - 1) {
						// 	$item.html(htmlStrings);
						// 	$item.attr('disable-Shorten', "true");
						// 	$item.siblings('.disclaimer__hide__single').hide();
						// 	$item.siblings('.disclaimer__show__single').hide();
				...
				var superscriptLength = function (string) {
					var len = 0,
						sup = string.substring(0,50).split("<sup").length - 1,
						b = string.substring(0,50).split("<b").length - 1;
					len += sup * 13 + b * 31;
					return len;
				};

				var swapText = function ($item) {
					var temp = $item.attr('data-default'),
						text = $item.html();
					$item.attr('data-default', text);
					$item.html(temp);
				};

				if($disclaimer_single && $disclaimer_single.length > 0) {
				...
				// 	_.forEach($disclaimer_single, function (item, idx) {
				// 		if($(item).siblings('.disclaimer__hide__single').length > 0 && ($($(item).siblings('.disclaimer__hide__single')[0]).css('display') === 'inline' || $($(item).siblings('.disclaimer__hide__single')[0]).css('display') === "inline-block")){
				// 			formatDisclaimer($(item), true, idx);
				// 		}
				// 		else if($(item).width() !== rowWidth) {
				// 			formatDisclaimer($(item), false, idx);
				// 		}
				// 	});
				// });
